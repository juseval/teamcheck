...
const EmployeesPage: React.FC<EmployeesPageProps> = ({ employees, onAddEmployee, onEditEmployee, onRemoveEmployee, workSchedules, currentUser, inviteCode }) => {
  ...
  const handleUpdateSchedule = async (employeeId: string, newScheduleId: string | null) => {
    ...
    if (!employee) return;
    try { await updateEmployeeDetails({ ...employee, workScheduleId: newScheduleId }); addNotification('Horario actualizado', 'success'); }
    ...
    <div className="w-full mx-auto animate-fade-in space-y-6 pb-8">
      ...
      {activeTab === 'board' && (
        <div className="h-[600px] min-h-[500px]">
          <EmployeeScheduleBoard employees={filteredEmployees} workSchedules={safeWorkSchedules} onUpdateEmployeeSchedule={handleUpdateSchedule}/>
        </div>
      )}
