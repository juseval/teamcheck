...
const ID_TYPE_SHORT: Record<string, string> = {
...
const EmployeesPage: React.FC<EmployeesPageProps> = ({ employees, onAddEmployee, onEditEmployee, onRemoveEmployee, workSchedules, currentUser, inviteCode }) => {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      try { const evs = await getCalendarEvents(); setAllEvents(evs); }
      catch(e) { console.error(e); }
    };
    loadEvents();
  }, [employees]);

  // ── FIX 2: wrapped in useMemo to satisfy react-hooks/exhaustive-deps ──
  const safeEmployees = useMemo(
    () => (Array.isArray(employees) ? employees : []),
    [employees]
  );
  const safeWorkSchedules = useMemo(
    () => (Array.isArray(workSchedules) ? workSchedules : []),
    [workSchedules]
  );
  const scheduleMap = useMemo(
    () => new Map(safeWorkSchedules.map(s => [s.id, s.name])),
    [safeWorkSchedules]
  );
  ...
  const handleExportCSV = () => {
    ...
    const rows = safeEmployees.map(emp => {
      ...
      return [emp.name, emp.email, emp.idType||'-', emp.idNumber||'-', emp.phone||'-', emp.location||'-', emp.role,
        emp.hireDate ? new Date(emp.hireDate).toLocaleDateString('es-CO') : '-',
        emp.workScheduleId ? (scheduleMap.get(emp.workScheduleId) ?? 'N/A') : 'N/A',
        emp.status, vac.accrued.toFixed(2), vac.taken.toFixed(1), vac.compensated.toFixed(1), vac.balance.toFixed(2), vac.maxCompensable.toFixed(2)];
    });
    ...
    a.download = `colaboradores_${new Date().toISOString().split('T')[0]}.csv`; a.click();
    URL.revokeObjectURL(url);
    addNotification('Exportación completada.', 'success');
  };

  const copyToClipboard = () => {
    navigator.clipboard.writeText(inviteCode || currentUser.companyId)
      .then(() => addNotification('Código copiado.', 'success'))
      .catch(() => addNotification('Error al copiar.', 'error'));
  };

  const handleUpdateSchedule = (employeeId: string, newScheduleId: string | null): void => {
    (async () => {
    ...
    <div className="w-full mx-auto animate-fade-in space-y-6 pb-8">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          ...
          <button onClick={onAddEmployee} className="bg-lucius-lime hover:bg-opacity-80 text-bokara-grey font-bold py-2 px-4 rounded-lg shadow-md flex items-center gap-2 cursor-pointer transition-all">
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {filteredEmployees.map(employee => {
                  ...
                  const idShort = employee.idType ? (ID_TYPE_SHORT[employee.idType] ?? employee.idType) : null;
                  return (
                    <tr key={employee.id} className="border-b border-bokara-grey/10 hover:bg-whisper-white/40 transition-colors">
                      ...
                      <td className="p-4 whitespace-nowrap">
                        {(idShort || employee.idNumber) ? (
                          <div className="flex items-center gap-1.5">
                            {idShort && (
                              <span className="text-[9px] font-black px-1.5 py-0.5 rounded border border-bokara-grey/15 bg-bokara-grey/5 text-bokara-grey/60 uppercase tracking-wide">
                                {idShort}
                              </span>
                            )}
                            <span className="text-xs font-mono text-bokara-grey/80">{employee.idNumber || '-'}</span>
                          </div>
                        ) : (
                          <span className="text-xs text-bokara-grey/25">—</span>
                        )}
                      </td>
                      ...
                      <td className="p-4 whitespace-nowrap text-xs text-bokara-grey/90">
                        {employee.workScheduleId ? scheduleMap.get(employee.workScheduleId) : 'N/A'}
                      </td>
                      <td className="p-4 whitespace-nowrap">
                      ...
                      <td className="p-4 whitespace-nowrap">
                        <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded-full ${employee.status === 'Working' ? 'bg-lucius-lime text-white' : 'bg-gray-100 text-gray-600'}`}>
                          {employee.status}
                        </span>
                      </td>
                      <td className="p-4 whitespace-nowrap">
      ...
      {/* ── FIX 1: onUpdateEmployeeSchedule tipado como (id, scheduleId | null) → void ──
          EmployeeScheduleBoard espera exactamente esa firma, no TrackerPageProps. */}
      {activeTab === 'board' && (
        ...
        <div className="fixed inset-0 bg-bokara-grey bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fade-in" onClick={() => setSelectedHistoryEmp(null)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-6 bg-whisper-white/30 border-b border-bokara-grey/10 flex justify-between items-center">
              ...
              <button onClick={() => setSelectedHistoryEmp(null)} className="p-2 text-bokara-grey/40 hover:text-bokara-grey text-2xl">&times;</button>
            </div>
            <div className="flex-grow p-6 overflow-y-auto">
              <div className="grid grid-cols-3 gap-3 mb-8">
                <div className="bg-gray-50 p-3 rounded-lg border text-center"><p className="text-[9px] font-bold text-gray-400 uppercase mb-1">Total Ganadas</p><p className="text-xl font-bold text-bokara-grey">{getVacationData(selectedHistoryEmp).accrued.toFixed(2)}</p></div>
                <div className="bg-lucius-lime/5 p-3 rounded-lg border border-lucius-lime/20 text-center"><p className="text-[9px] font-bold text-lucius-lime uppercase mb-1">Disfrutadas</p><p className="text-xl font-bold text-bokara-grey">-{getVacationData(selectedHistoryEmp).taken.toFixed(1)}</p></div>
                <div className="bg-wet-sand/5 p-3 rounded-lg border border-wet-sand/20 text-center"><p className="text-[9px] font-bold text-wet-sand uppercase mb-1">Compensadas</p><p className="text-xl font-bold text-bokara-grey">-{getVacationData(selectedHistoryEmp).compensated.toFixed(1)}</p></div>
              </div>
