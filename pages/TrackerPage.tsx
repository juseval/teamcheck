...
interface TrackerPageProps {
  ...
  // ── NUEVO: eliminar registro del log (solo admins) ──
...
const TrackerPage: React.FC<TrackerPageProps> = ({ 
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logsToFilter.filter(log => {
      ...
      return (startTimestamp === 0 || logTs >= startTimestamp) &&
             (endTimestamp === Infinity || logTs <= endTimestamp);
    });
                  ...
                  <div key={schedule.id}>
                    ...
                    <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                      {employeesBySchedule[schedule.id].map(employee => (
                        <EmployeeCard key={employee.id} employee={employee} onAction={onEmployeeAction} onRemove={onRemoveEmployee} onEditTime={onEditTime} activityStatuses={activityStatuses} userRole={userRole} />
                      ))}
                    </div>
                  </div>
                )
