...
import { getNotificationRecipients, addNotificationRecipient, removeNotificationRecipient, getEmailConfig, saveEmailConfig, updateCompanyName } from '../services/apiService';
...
interface SettingsPageProps {
  ...
  onAddPayrollChangeType: (
    name: string, color: string, requiereAprobacion: boolean, soloAdmin: boolean,
    yearlyQuota?: number, semesterQuota?: number
  ) => Promise<void>;
  ...
  onCompanyRenamed?: (newName: string) => void;
}

// ── URL pública del instalador .exe ──────────────────────────────────────────
// Cambia esta constante por el link de Google Drive / GitHub Releases / S3
// cuando hayas subido el instalador generado con: npm run electron:build:win
const ELECTRON_DOWNLOAD_URL = '';
...
const DayPicker: React.FC<{ selectedDays: number[]; onDayToggle: (dayIndex: number) => void }> = ({ selectedDays, onDayToggle }) => (
  <div className="flex justify-center gap-1">
    {dayLabels.map((label, index) => {
      ...
      return (
        <button key={index} type="button" onClick={() => onDayToggle(index)}
          className={`w-8 h-8 rounded-md text-sm font-bold transition-colors ${isSelected ? 'bg-lucius-lime text-bokara-grey' : 'bg-bokara-grey/10 text-bokara-grey/60 hover:bg-bokara-grey/20'}`}>
          {label}
        </button>
      );
    })}
...
const ToggleCheck: React.FC<{ checked: boolean; onChange: (v: boolean) => void; label: string; sublabel?: string }> = ({ checked, onChange, label, sublabel }) => (
  <button type="button" onClick={() => onChange(!checked)}
    className={`flex items-start gap-2 p-2 rounded-lg border transition-all cursor-pointer text-left w-full ${checked ? 'bg-lucius-lime/10 border-lucius-lime/40' : 'bg-bright-white border-bokara-grey/15 hover:border-bokara-grey/30'}`}>
    <div className={`mt-0.5 w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${checked ? 'bg-lucius-lime border-lucius-lime' : 'bg-white border-bokara-grey/30'}`}>
      {checked && <svg className="w-2.5 h-2.5 text-bokara-grey" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
    </div>
    <div>
...
// ── Banner de descarga de Electron ────────────────────────────────────────────
const downloadSteps = [
  { icon: '⬇️', title: 'Descarga el instalador', desc: 'Haz clic en el botón y guarda el archivo .exe.' },
  { icon: '🛡️', title: 'Ejecuta como administrador', desc: 'Si Windows muestra "SmartScreen", haz clic en "Más información" → "Ejecutar de todas formas".' },
  { icon: '🔔', title: 'Arranca minimizado', desc: 'La app aparece en la bandeja del sistema. Haz doble clic para abrirla.' },
  { icon: '✅', title: 'Inicia sesión y ficha', desc: 'Entra con tu cuenta de TeamCheck. Las capturas y el control de inactividad se activan al hacer Clock In.' },
];

const DownloadElectronBanner: React.FC<{ downloadUrl?: string }> = ({ downloadUrl }) => {
  if (isElectron) {
    return (
      <div className="rounded-2xl border border-lucius-lime/40 bg-lucius-lime/10 p-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Las capturas de pantalla y la detección de inactividad están activas.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-bokara-grey px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {downloadUrl ? (
          ...
          <span className="text-white/40 text-sm italic">
            Configura <code className="bg-white/10 px-1 rounded text-xs">ELECTRON_DOWNLOAD_URL</code> en SettingsPage
          </span>
        )}
      </div>
...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  companyId, inviteCode, companyName, currentUserRole, onCompanyRenamed,
}) => {
  ...
  useEffect(() => { setNewCompanyName(companyName || ''); }, [companyName]);

  const handleRenameCompany = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCompanyName.trim() || !companyId || isSavingName) return;
    setIsSavingName(true);
    try {
      await updateCompanyName(companyId, newCompanyName.trim());
      onCompanyRenamed?.(newCompanyName.trim());
      addNotification('Nombre de empresa actualizado.', 'success');
      setEditingName(false);
    } catch { addNotification('Error al actualizar el nombre.', 'error'); }
  ...
  const handlePayrollChangeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onAddPayrollChangeType(
        newPayrollChangeName.trim(), newPayrollChangeColor,
        newPayrollRequiereAprobacion, newPayrollSoloAdmin,
        newPayrollQuota > 0 ? newPayrollQuota : undefined,
        newPayrollSemesterQuota > 0 ? newPayrollSemesterQuota : undefined,
      );
      setNewPayrollChangeName(''); setNewPayrollChangeColor('#3B82F6');
      setNewPayrollRequiereAprobacion(false); setNewPayrollSoloAdmin(false);
      setNewPayrollQuota(0); setNewPayrollSemesterQuota(0);
  ...
  const handleAddRecipient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification('Correo agregado.', 'success');
  ...
  const handleTestEmail = async () => {
    if (!emailConfig.serviceId || !emailConfig.templateId || !emailConfig.publicKey) { addNotification('Primero completa todos los campos de EmailJS.', 'error'); return; }
    if (notifRecipients.length === 0) { addNotification("Agrega al menos un correo en 'Notificaciones de RRHH'.", 'error'); return; }
    ...
    try {
      const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
        ...
        body: JSON.stringify({
          service_id: emailConfig.serviceId, template_id: emailConfig.templateId, user_id: emailConfig.publicKey,
          template_params: { recipient_email: notifRecipients[0], user_name: 'Soporte TeamCheck', request_type: 'PRUEBA DE CONEXIÓN', start_date: new Date().toLocaleDateString(), end_date: new Date().toLocaleDateString() },
        }),
      });
      if (response.ok) addNotification(`¡Prueba exitosa! Revisa ${notifRecipients[0]}`, 'success');
      else throw new Error(await response.text());
    } catch (error: any) { addNotification(`Error de EmailJS: ${error.message}`, 'error'); }
    ...
    <div className="w-full max-w-6xl mx-auto animate-fade-in space-y-8 pb-10">
      ...
      {activeSection === 'general' ? (
        <div className="space-y-8">
          ...
          {/* RENOMBRAR EMPRESA (solo master) */}
          {currentUserRole === 'master' && companyId && (
            <div className="bg-white rounded-xl shadow-md border border-bokara-grey/10 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-bokara-grey">Nombre de la Empresa</h2>
                  <p className="text-xs text-bokara-grey/50">Solo visible para propietarios (Master)</p>
                </div>
                <span className="ml-auto text-[9px] bg-amber-100 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-full font-black uppercase">⭐ Master</span>
              </div>

              {!editingName ? (
                <div className="flex items-center justify-between bg-whisper-white/60 rounded-xl px-5 py-4 border border-bokara-grey/10">
                  <div>
                    <p className="text-xs text-bokara-grey/40 font-bold uppercase tracking-widest mb-0.5">Nombre actual</p>
                    <p className="text-2xl font-bold text-bokara-grey">{companyName || '—'}</p>
                  </div>
                  <button onClick={() => setEditingName(true)} className="flex items-center gap-2 px-4 py-2 bg-bokara-grey text-white font-bold rounded-lg text-sm hover:bg-black transition-all">
                ...
                <form onSubmit={handleRenameCompany} className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 border border-bokara-grey/20 rounded-xl px-4 py-3 text-bokara-grey font-bold focus:outline-none focus:ring-2 focus:ring-lucius-lime text-lg"
                  />
                  <button type="submit" disabled={isSavingName || !newCompanyName.trim()} className="px-5 py-3 bg-lucius-lime text-bokara-grey font-bold rounded-xl hover:bg-opacity-80 transition-all disabled:opacity-50">
                    {isSavingName ? 'Guardando...' : 'Guardar'}
                  </button>
                  <button type="button" onClick={() => { setEditingName(false); setNewCompanyName(companyName || ''); }} className="px-4 py-3 bg-gray-100 text-bokara-grey font-bold rounded-xl hover:bg-gray-200 transition-all">
                    Cancelar
                  </button>
                </form>
              )}
          ...
          <div className="bg-lucius-lime rounded-2xl shadow-xl p-8 border border-bokara-grey/5 relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <button
                  onClick={() => { if (inviteCode) { navigator.clipboard.writeText(inviteCode); addNotification('¡Código copiado!', 'success'); } }}
              ...
              <div className="space-y-2">
                {statuses.map(s => (
                  <div key={s.id} className="flex items-center justify-between bg-whisper-white/60 p-3 rounded-lg">
                    ...
                    <button onClick={() => onRemoveStatus(s.id)} className="text-red-500 hover:bg-red-500/10 p-1 rounded-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <form onSubmit={handlePayrollChangeSubmit} className="bg-whisper-white/60 p-4 rounded-lg border border-bokara-grey/10 space-y-4">
                  <div><label className="block text-sm font-medium text-bokara-grey mb-1">Nombre de la Novedad</label><input type="text" value={newPayrollChangeName} onChange={e => setNewPayrollChangeName(e.target.value)} className="w-full bg-bright-white border border-bokara-grey/20 text-bokara-grey rounded-lg px-3 py-2" required /></div>
                  <div><label className="block text-sm font-medium text-bokara-grey mb-1">Color</label><input type="color" value={newPayrollChangeColor} onChange={e => setNewPayrollChangeColor(e.target.value)} className="w-full h-10 p-1 bg-bright-white border border-bokara-grey/20 rounded-lg" /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-xs font-bold text-bokara-grey/60 uppercase tracking-wide mb-1">Cupo Anual</label><input type="number" min="0" value={newPayrollQuota} onChange={e => setNewPayrollQuota(Number(e.target.value))} className="w-full bg-bright-white border border-bokara-grey/20 text-bokara-grey rounded-lg px-3 py-2 text-sm" placeholder="0 = sin límite" /><p className="text-[9px] text-bokara-grey/40 mt-0.5">Máx. usos por año</p></div>
                    <div><label className="block text-xs font-bold text-bokara-grey/60 uppercase tracking-wide mb-1">Cupo Semestral</label><input type="number" min="0" value={newPayrollSemesterQuota} onChange={e => setNewPayrollSemesterQuota(Number(e.target.value))} className="w-full bg-bright-white border border-bokara-grey/20 text-bokara-grey rounded-lg px-3 py-2 text-sm" placeholder="0 = sin límite" /><p className="text-[9px] text-bokara-grey/40 mt-0.5">Activa ventanas S1/S2</p></div>
                  </div>
                  <div className="space-y-2">
                    <ToggleCheck checked={newPayrollRequiereAprobacion} onChange={setNewPayrollRequiereAprobacion} label="Requiere Aprobación" sublabel="El colaborador solicita y el admin aprueba antes de que cuente en la tabla anual." />
                    <ToggleCheck checked={newPayrollSoloAdmin} onChange={setNewPayrollSoloAdmin} label="Solo Admin" sublabel="El admin lo registra directamente; el colaborador no puede solicitarlo." />
                  </div>
                  <button type="submit" disabled={isPayrollSubmitting} className="w-full bg-lucius-lime hover:bg-opacity-80 text-bokara-grey font-bold py-2 px-4 rounded-lg disabled:opacity-50">{isPayrollSubmitting ? 'Guardando...' : 'Agregar Tipo'}</button>
                </form>
              </div>
                  ...
                  return (
                    <div key={type.id} className="bg-whisper-white/60 p-3 rounded-lg border border-bokara-grey/10 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => onRemovePayrollChangeType(type.id)} className="text-red-400 hover:text-red-600 hover:bg-red-50 p-1 rounded-md transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="flex items-center gap-1.5"><label className="text-[10px] font-bold text-bokara-grey/50 uppercase whitespace-nowrap">Cupo Anual:</label><input type="number" min="0" defaultValue={type.yearlyQuota ?? 0} key={`ya-${type.id}-${type.yearlyQuota}`} onBlur={e => { const v = e.target.value === '' ? undefined : Number(e.target.value); onUpdatePayrollChangeType(type.id, { yearlyQuota: v }); }} className="w-14 bg-white border border-bokara-grey/10 rounded px-1 py-0.5 text-xs" /></div>
                        <div className="flex items-center gap-1.5"><label className="text-[10px] font-bold text-bokara-grey/50 uppercase whitespace-nowrap">Cupo Sem.:</label><input type="number" min="0" defaultValue={type.semesterQuota ?? 0} key={`sq-${type.id}-${type.semesterQuota}`} onBlur={e => { const v = e.target.value === '' ? undefined : Number(e.target.value); onUpdatePayrollChangeType(type.id, { semesterQuota: v }); }} className="w-14 bg-white border border-bokara-grey/10 rounded px-1 py-0.5 text-xs" /></div>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <button type="button" onClick={() => onUpdatePayrollChangeType(type.id, { requiereAprobacion: !reqAprobacion, isExclusive: !reqAprobacion })} className={`flex items-center gap-1.5 p-1.5 rounded-md border cursor-pointer transition-all text-[10px] font-bold ${reqAprobacion ? 'bg-lucius-lime/10 border-lucius-lime/30 text-bokara-grey' : 'bg-white border-bokara-grey/10 text-bokara-grey/50'}`}>
                          <div className={`w-3 h-3 rounded border flex items-center justify-center flex-shrink-0 ${reqAprobacion ? 'bg-lucius-lime border-lucius-lime' : 'border-bokara-grey/30'}`}>{reqAprobacion && <svg className="w-2 h-2 text-bokara-grey" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}</div>
                          Req. Aprobación
                        </button>
                        <button type="button" onClick={() => onUpdatePayrollChangeType(type.id, { soloAdmin: !soloAdm, adminOnly: !soloAdm })} className={`flex items-center gap-1.5 p-1.5 rounded-md border cursor-pointer transition-all text-[10px] font-bold ${soloAdm ? 'bg-bokara-grey/10 border-bokara-grey/30 text-bokara-grey' : 'bg-white border-bokara-grey/10 text-bokara-grey/50'}`}>
                          <div className={`w-3 h-3 rounded border flex items-center justify-center flex-shrink-0 ${soloAdm ? 'bg-bokara-grey border-bokara-grey' : 'border-bokara-grey/30'}`}>{soloAdm && <svg className="w-2 h-2 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}</div>
                          Solo Admin
                        </button>
                      </div>
                      {(type.semesterQuota || type.yearlyQuota) ? (
                        <div className="flex flex-wrap gap-1">
                          {type.semesterQuota
                            ? <span className="text-[9px] bg-blue-50 text-blue-600 border border-blue-100 px-1.5 py-0.5 rounded-full font-bold">📅 {type.semesterQuota}/semestre → ventana S1: dic–may · S2: jun–nov</span>
                            : <span className="text-[9px] bg-amber-50 text-amber-600 border border-amber-100 px-1.5 py-0.5 rounded-full font-bold">📆 {type.yearlyQuota}/año</span>
                          }
                        </div>
                      ) : null}
                    </div>
        ...
        <div className="space-y-8 animate-fade-in">
          ...
          {/* ── TEAMCHECK DESKTOP (Electron) ── */}
          <div className="bg-white rounded-xl shadow-md border border-bokara-grey/10 p-6">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-bokara-grey rounded-lg">
                <svg className="w-6 h-6 text-lucius-lime" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
              </div>
              <div>
                <h2 className="text-2xl font-bold text-bokara-grey">App de Escritorio</h2>
                <p className="text-bokara-grey/50 text-sm">Capturas automáticas · Detección de inactividad · Auto Clock Out al cerrar PC</p>
              </div>
            </div>
            <DownloadElectronBanner downloadUrl={ELECTRON_DOWNLOAD_URL || undefined} />
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md border border-bokara-grey/10 p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <form onSubmit={handleSaveEmailConfig} className="space-y-4">
                <div><label className="block text-[10px] font-bold text-bokara-grey/40 uppercase tracking-widest mb-1">Service ID <span className="text-red-500">*</span></label><input type="text" value={emailConfig.serviceId} onChange={e => setEmailConfig({ ...emailConfig, serviceId: e.target.value })} className="w-full bg-whisper-white border border-bokara-grey/20 rounded-lg p-2 text-sm focus:ring-2 focus:ring-lucius-lime outline-none" placeholder="service_xxxxxx" required /></div>
                <div><label className="block text-[10px] font-bold text-bokara-grey/40 uppercase tracking-widest mb-1">Template ID <span className="text-red-500">*</span></label><input type="text" value={emailConfig.templateId} onChange={e => setEmailConfig({ ...emailConfig, templateId: e.target.value })} className="w-full bg-whisper-white border border-bokara-grey/20 rounded-lg p-2 text-sm focus:ring-2 focus:ring-lucius-lime outline-none" placeholder="template_xxxxxx" required /></div>
                <div><label className="block text-[10px] font-bold text-bokara-grey/40 uppercase tracking-widest mb-1">Public Key <span className="text-red-500">*</span></label><input type="text" value={emailConfig.publicKey} onChange={e => setEmailConfig({ ...emailConfig, publicKey: e.target.value })} className="w-full bg-whisper-white border border-bokara-grey/20 rounded-lg p-2 text-sm focus:ring-2 focus:ring-lucius-lime outline-none" placeholder="user_xxxxxx" required /></div>
                <div className="grid grid-cols-2 gap-3">
                  <button type="submit" disabled={isSavingEmail} className="bg-bokara-grey text-white font-bold py-3 rounded-lg hover:bg-black transition-all text-sm">{isSavingEmail ? 'Guardando...' : 'Guardar Credenciales'}</button>
                  <button type="button" onClick={handleTestEmail} disabled={isTestingEmail || !emailConfig.publicKey} className="bg-lucius-lime text-bokara-grey font-bold py-3 rounded-lg hover:bg-opacity-80 transition-all text-sm flex items-center justify-center gap-2">
                    {isTestingEmail
                      ? <div className="w-4 h-4 border-2 border-bokara-grey border-t-transparent rounded-full animate-spin" />
                      : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Probar Conexión</>
                    }
                  </button>
                </div>
              </form>
