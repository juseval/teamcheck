...
const AppContent: React.FC = () => {
    ...
    const [currentPage, setCurrentPage] = useState(() => {
        return sessionStorage.getItem('currentPage') || 'tracker';
    });
    ...
    // Guardar isAuthenticated en sessionStorage cuando cambie
    useEffect(() => {
        sessionStorage.setItem('isAuthenticated', isAuthenticated.toString());
    }, [isAuthenticated]);

    // Guardar currentPage en sessionStorage cuando cambie
    useEffect(() => {
        sessionStorage.setItem('currentPage', currentPage);
    }, [currentPage]);

    // Guardar loggedInUser en sessionStorage cuando cambie
    useEffect(() => {
        if (loggedInUser) {
    ...
    const handleLogin = async (credentials: { email: string; }) => {
        try {
            ...
            if (result.success && result.user) {
                ...
                const initialPage = result.user.role === 'admin' ? 'dashboard' : 'tracker';
