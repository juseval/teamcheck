  ...
  const processedData = useMemo(() => {
    ...
    logsByEmployee.forEach((logs, employeeId) => {
      ...
      for (let i = 0; i < logs.length - 1; i++) {
        if (logs[i].action.startsWith('Start '))
          totalBreakSeconds += (logs[i + 1].timestamp - logs[i].timestamp) / 1000;
      }
          ...
          {processedData.structuredData.map(({ schedule, employees: employeeList }) => (
            <tbody key={schedule.id} className="border-t-2 border-bokara-grey/10">
              ...
              {employeeList.map(employee => {
                ...
                // Novedad de hoy (aprobada > pendiente)
                ...
                return (
                  <tr key={employee.id} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      {firstClockIn ? (
                        // Marcó → Active / Done + novedad al lado si la tiene
                        ...
                        // No marcó pero tiene novedad → mostrar la novedad en lugar de "Absent"
                        EventBadge
                      ) : (
                        // Sin marcación y sin novedad → Absent
