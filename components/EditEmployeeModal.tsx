                ...
                <div className="space-y-4">
                  {['admin', 'employee'].map(role => (
                    <label key={role} className="flex items-start gap-3 cursor-pointer group">
                      <input type="radio" name="role" value={role} checked={employeeData.role === role} onChange={handleChange} className="mt-1 text-lucius-lime focus:ring-lucius-lime" />
                      <div>
                        <p className="font-bold text-gray-700 capitalize">{role === 'admin' ? 'Administrador' : 'Colaborador'}</p>
                        <p className="text-xs text-gray-400">{role === 'admin' ? 'Tiene acceso total a la gestión de empleados y configuración.' : 'Usuario estándar con acceso solo a su propio perfil y marcaciones.'}</p>
                      </div>
                    </label>
                  ))}
                </div>
