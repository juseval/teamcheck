...
interface EditTimesheetEntryModalProps {
  isOpen: boolean;
  onClose: () => void;
  // ── FIX: startLogId y endLogId son string en TimesheetEntry, no number ──
  onSave: (startLogId: string, endLogId: string, newStartTime: number, newEndTime: number) => void;
...
const EditTimesheetEntryModal: React.FC<EditTimesheetEntryModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // ── Pasar como string directamente — son IDs de documentos Firestore ──
    onSave(entry.startLogId, entry.endLogId, newStartDate.getTime(), newEndDate.getTime());
  };

  return (
    <div
      className="fixed inset-0 bg-bokara-grey bg-opacity-60 flex items-center justify-center z-50 transition-opacity"
      onClick={onClose}
      aria-modal="true"
      role="dialog"
    >
      <div
        className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md border border-bokara-grey/10"
        onClick={e => e.stopPropagation()}
      >
        <h2 className="text-2xl font-bold text-bokara-grey mb-2">Edit Timesheet Entry</h2>
        <p className="text-bokara-grey/80 mb-4">
          For <span className="font-semibold">{entry.employeeName}</span> on{' '}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="startTime" className="block text-sm font-medium text-bokara-grey mb-1">
                Start Time
              </label>
              <input
                id="startTime"
            ...
            <div>
              <label htmlFor="endTime" className="block text-sm font-medium text-bokara-grey mb-1">
                End Time
              </label>
              <input
                id="endTime"
          ...
          {!isValid && startTime && endTime && (
            <p className="text-red-500 text-sm text-center">End time must be after start time.</p>
          )}
          ...
          <div className="pt-4 flex justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="py-2 px-4 bg-gray-200 text-bokara-grey rounded-lg hover:bg-gray-300 transition-colors font-semibold"
            >
              Cancel
            </button>
            <button
